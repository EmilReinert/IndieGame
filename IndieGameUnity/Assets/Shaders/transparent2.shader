Shader "Emil/transparent tex swap"
{
	Properties
	{
		...
		_A2("Global Alpha", Range(0,1)) = 0.5
	}
		SubShader
	...
	#include "UnityCG.cginc"
			...
			SHADERDATA vert(float4 vertex:POSITION, float2 uv : TEXCOORD0, float2 uv2 : TEXCOORD0)
			{
				...
				vs.uv = TRANSFORM_TEX(uv, _SecTex);
				vs.uv2 = TRANSFORM_TEX(uv2,  _MainTex);
			...
			float4 col = _Color * (tex2D( _SecTex, ps.uv)*(_TexStr2)) + (tex2D(_MainTex, ps.uv2)*(_TexStr));
			// 
